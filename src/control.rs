...
impl UpgradeAction {
    /// Take the upgrade action, returning either `Some(RefPtr<T>)` or `None`
    /// depending on whether or not the upgrade succeeded.
...
impl FreeAction {
    /// Take the free action, freeing the value behind `ptr` if `FreeMemory` was
    /// returned.
...
/// Common trait implemented by various control block types.
